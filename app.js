...
store.leads.forEach(l => { if (!l.contactRole) l.contactRole = ''; if (!l.contactPhone) l.contactPhone = ''; if (l.convertedProjectId === undefined) l.convertedProjectId = null; if (!l.nextAction) l.nextAction = ''; if (!l.nextActionDate) l.nextActionDate = ''; });
  ...
  if (sows.length === 0) {
    ...
    sows.forEach(sow => {
      ...
      html += '<div class="fl-card">'
        ...
        + (canInvoice && sowInvoices.length === 0 ? '<button class="btn btn-primary btn-sm" onclick="flSubmitInvoiceModal(\'' + sow.id + '\',\'' + (sow.project_id || '') + '\',' + (sow.rate || 0) + ',' + timeEntries.filter(e=>e.sow_id===sow.id&&!e.invoice_id).reduce((s,e)=>s+(e.hours||0),0) + ')">Submit Invoice</button>' : '')
